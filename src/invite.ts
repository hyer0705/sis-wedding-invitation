...
export const INVITE = {
  ...
  // RS-01~03 — 폼 구성과 전송은 SIS-15·SIS-20이 다룬다.
  // 식사 여부는 명세서에서 추가된 항목이고, 3택으로 확정됐다(2026-08-18).
  // 연락처는 고객 요청으로 수집한다(2026-08-18) — 명세서에는 「미수집」으로 남아
  // 있어 시트 갱신이 필요하다. 수집 항목이 늘었으므로 개인정보 동의 문구(RS-03)에
  // 연락처가 반드시 포함되어야 한다.
  //
  // RSVP 섹션은 v1 에서 빠져 있다(2026-08-13, App.tsx 참고). 여기 남은 것은 시트에서
  // 확정받은 값뿐이고, 팝업 문구와 개인정보 동의 문구(RS-03)는 mock 이었기에 지웠다 —
  // 남겨 두면 isMock 을 true 로 되돌려야 해서 배포가 막힌다. 되살릴 때는 문구를 다시
  // 받는다. RS-03 은 법적 요구사항이라 임의로 지어 넣지 않는다.
  rsvp: {
