...
describe("문지기", () => {
  ...
  it("토큰이 갱신돼도 화면과 걸어 둔 필터를 지킨다", async () => {
    const user = userEvent.setup();
    listMock.mockResolvedValue([
      row({ id: "a", name: "김민준", attend: "참석" }),
      row({ id: "b", name: "이서연", attend: "미참석" }),
    ]);
    ...
    notifyAuth?.(SESSION);
    await waitFor(() => expect(adminMock).toHaveBeenCalledTimes(2));

    expect(listMock).toHaveBeenCalledTimes(1);
    expect(screen.getAllByRole("listitem")).toHaveLength(1);
    expect(screen.getByRole("button", { name: "참석 1" })).toHaveAttribute("aria-pressed", "true");
  });
