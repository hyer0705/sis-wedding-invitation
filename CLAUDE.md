...
- **배경음악은 켜진 채로 시작한다** (2026-09-04 고객 요청, SIS-51 — 그전까지는 음소거로 시작했다). 다만 **소리 있는 오디오의 자동재생은 어떤 브라우저도 허용하지 않는다** — `play()` 가 `NotAllowedError` 로 거절되며 코드로 우회할 수 없다. 그래서 마운트 직후 한 번 시도하고, 막히면 토글을 켜 둔 채 **첫 사용자 제스처(`pointerdown`·`touchend`·`keydown`)를 한 번만 기다렸다가 그 자리에서 재생한다**. 제스처가 있는데도 막히면(카카오톡 인앱 등) 그때는 토글을 꺼진 모양으로 되돌린다 — 켜진 채 조용하면 하객이 자기 기기 문제로 여긴다. **토글 자신은 그 대기에서 뺀다**: 막힌 상태에서 토글을 누르면 `pointerdown` 의 되돌림과 `click` 의 토글이 겹쳐 끄려던 것이 오히려 켜진다
