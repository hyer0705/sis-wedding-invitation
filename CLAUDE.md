...
- **우상단 고정 버튼은 둘이다** — 「큰 글씨로 보기」와 배경음악(`.floating-controls`). 큰 글씨 토글은 커버 아래에도 글자 버튼으로 한 번 더 있는데(2026-09-01 확정), 우상단 원만 두면 그것이 무엇을 하는 버튼인지 알기 어렵고 이 기능이 필요한 하객일수록 작은 아이콘을 짚어 보지 않는다. 두 버튼은 같은 상태를 본다
