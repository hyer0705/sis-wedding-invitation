...
| **M3 인터랙션·검수** | 8/5 (수) | 계좌·안내·공유·공통 UX·품질 게이트·릴리스 (~~RSVP·BGM~~ → v1.1) |
...
**v1 에서 뺀 것 (2026-08-13 결정)**: RSVP(`SIS-15`)와 배경음악(`SIS-27`)을 M3 범위에서 v1.1 로 옮겼다. BGM 은 고객이 음원 저작권을 확인 중이고, RSVP 는 구현이 스텁이라 자리표시 문구밖에 없었다. **둘 다 기다리면 배포가 무기한 밀리고, 청첩장을 공유하지 못하는 비용이 더 크다고 판단했다.** RSVP 는 문구 3건(팝업·개인정보 동의·「전하고 싶은 말」 유지 여부)을 받는 대로 착수한다.
